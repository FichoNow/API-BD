...
import { PatchFichajeEntryEndBody } from "../../../types/dto/user/fichaje-entries/patch-fichaje-entry-end-body.js";
...
export async function updateFichajeEntryEndService(
  ...
  if (body.ended_at < toMinute(entry.started_at)) {
  ...
  if (body.ended_at < toMinute(fichaje.clock_in)) {
    throw new ResponseError(
